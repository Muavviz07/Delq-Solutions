// src/pages/services/GenericServicePage.js
...
// --- Helper Component for list items with red diamond ---
    ...
    switch (block.type) {
        ...
        case 'paragraph':
            ...
            return (
                <p
                    // --- MODIFIED: Increased font size from text-sm to text-base ---
                    className={`text-[#4d4d4d] leading-relaxed text-base py-1 ${containsLink ? 'service-link-paragraph' : ''}`}
                    dangerouslySetInnerHTML={{ __html: block.content }}
                />
            );
        case 'image':
            // Standard images (not the first one) keep h-96
            return (
                <img
                    src={block.image_url}
                    alt={block.alt_text || 'Service content image'}
        ...
        case 'image_list':
            return (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    <img
                        src={block.image_url}
                        alt={block.alt_text || 'Service info'}
                        className="w-full h-80 object-cover rounded-lg shadow-md" // Keep h-80
                    />
                    <ul className="space-y-4">
                        {/* Only slice image_list items */}
        ...
        case 'accordion_section':
            // Check if heading exists and is not empty before rendering it
            ...
            return (
                <section>
                    {showHeading && (
                        <h2
                            // --- MODIFIED: Increased font size from text-3xl to text-4xl ---
                            className={`text-4xl font-bold mb-2 ${block.heading.includes("WHY CHOOSE") ? 'text-[#d90a2c]' : 'text-black'}`}
                            style={{ fontFamily: "'Urbanist', sans-serif" }}
                         >
                            {block.heading}
                         </h2>
                    ...
                    {block.subheading && (
                        // --- MODIFIED: Increased font size from text-sm to text-base ---
                        <p className="text-[#4d4d4d] leading-relaxed mb-2 text-base py-1">
                            {block.subheading}
                        </p>
                    )}
...
// --- Main Page Component ---
export default function GenericServicePage({ fetchUrl, breadcrumbOverride = null }) {
    ...
    const breadcrumbs = breadcrumbOverride || pageData.breadcrumbs || [];
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 sm:gap-12 md:gap-16 lg:gap-20 xl:gap-24">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        {(pageData.content_blocks || []).map((block, index) => (
                            // Check for first image block and apply larger height
                            index === 0 && block.type === 'image' ? (
                                ...
                                // Render other blocks normally using the ContentBlock helper
